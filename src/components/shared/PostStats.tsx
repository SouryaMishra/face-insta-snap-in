...
const PostStats = ({ post, userId }: IPostStatsProps) => {
  ...
  const { mutateAsync: deleteSavedPost, isPending: isDeletingPost } = useDeleteSavedPost();

  const { data: currentUser, isPending: isGettingCurrentUser } = useGetCurrentUser();
  ...
  return (
    <div className={`flex justify-between items-center z-20`}>
      ...
      <div className="flex gap-2">
        <button onClick={(e) => handleSavePost(e)} disabled={isSavingPost || isDeletingPost}>
          <img
