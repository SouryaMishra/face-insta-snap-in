...
import type { INewPost, INewUser, INewUserRecord, IUpdatePost, IUpdateUser } from "@/types";
...
export async function getPostByIds(postIds?: string[]) {
  if (!postIds) throw Error;
  if (postIds.length === 0) throw Error;
